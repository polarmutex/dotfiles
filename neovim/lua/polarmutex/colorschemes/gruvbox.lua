...
local theme = lush(function()
    return {
        ...
        GruvboxFg0({fg = fg0}), GruvboxFg1({fg = fg1}), GruvboxFg2({fg = fg2}),
        GruvboxFg3({fg = fg3}), GruvboxFg4({fg = fg4}), GruvboxGray({fg = gray}),
        GruvboxBg0({fg = bg0}), GruvboxBg1({fg = bg1}), GruvboxBg2({fg = bg2}),
        GruvboxBg3({fg = bg3}), GruvboxBg4({fg = bg4}), GruvboxRed({fg = red}),
        GruvboxRedBold({fg = red, gui = styles.bold}), GruvboxGreen({fg = green}),
        GruvboxGreenBold({fg = green, gui = styles.bold}), GruvboxYellow({fg = yellow}),
        GruvboxYellowBold({fg = yellow, gui = styles.bold}), GruvboxBlue({fg = blue}),
        GruvboxBlueBold({fg = blue, gui = styles.bold}), GruvboxPurple({fg = purple}),
        GruvboxPurpleBold({fg = purple, gui = styles.bold}), GruvboxAqua({fg = aqua}),
        GruvboxAquaBold({fg = aqua, gui = styles.bold}), GruvboxOrange({fg = orange}),
        ...
        ColorColumn({bg = color_column}), Conceal({fg = blue}),
        Cursor({gui = styles.inverse}), lCursor({Cursor}), iCursor({Cursor}),
        vCursor({Cursor}), CursorIM({Cursor}), CursorLine({bg = bg1}),
        CursorColumn({CursorLine}), Directory({GruvboxRedBold}),
        ...
        FoldColumn({fg = gray, bg = bg1}), SignColumn({bg = sign_column}),
        IncSearch({fg = hls_cursor, bg = bg0, gui = styles.inverse}),
        LineNr({fg = bg4, bg = number_column}), CursorLineNr({fg = yellow, bg = bg1}),
        MatchParen({bg = bg3, gui = styles.bold}), ModeMsg({GruvboxYellowBold}),
        MoreMsg({GruvboxYellowBold}), NonText({GruvboxBg2}),
        Normal({fg = fg1, bg = bg0}), NormalFloat({Normal}), NormalNC({Normal}),
        Pmenu({fg = fg1, bg = bg2}), PmenuSel({fg = bg2, bg = blue, gui = styles.bold}),
        PmenuSbar({bg = bg2}), PmenuThumb({bg = bg4}), Question({GruvboxOrangeBold}),
        QuickFixLine({fg = bg0, bg = yellow, gui = styles.bold}),
        Search({fg = hls_highlight, bg = bg0, gui = styles.inverse}),
        SpecialKey({GruvboxFg4}), SpellRare({GruvboxPurpleUnderline}),
        ...
        Title({GruvboxGreenBold}), Visual({bg = bg3, gui = invert_selection}),
        VisualNOS({Visual}), WarningMsg({GruvboxRedBold}),
        WildMenu({fg = blue, bg = bg2, gui = styles.bold}), Constant({GruvboxPurple}),
        ...
        }), Character({GruvboxPurple}), Number({GruvboxPurple}),
        Boolean({GruvboxPurple}), Float({GruvboxPurple}), Identifier({GruvboxBlue}),
        Function({GruvboxGreenBold}), Statement({GruvboxRed}),
        Conditional({GruvboxRed}), Repeat({GruvboxRed}), Label({GruvboxRed}),
        Exception({GruvboxRed}), Keyword({GruvboxRed}), Operator({GruvboxFg1}),
        PreProc({GruvboxAqua}), Include({GruvboxAqua}), Define({GruvboxAqua}),
        Macro({GruvboxAqua}), PreCondit({GruvboxAqua}), Type({GruvboxYellow}),
        StorageClass({GruvboxOrange}), Structure({GruvboxAqua}),
        Typedef({GruvboxYellow}), SpecialChar({GruvboxRed}), Tag({GruvboxAquaBold}),
        Delimiter({GruvboxFg0}), -- Comment {fg = gray, gui = italic_comments},
        Comment({fg = gray}), Debug({GruvboxRed}),
        Underlined({fg = blue, gui = styles.underline}), Bold({gui = styles.bold}),
        Italic({gui = styles.italic}), Ignore({}),
        Error({fg = red, gui = styles.bold .. "," .. styles.inverse}),
        -- Todo {fg = fg0, gui = styles.bold .. "," .. italic_comments},
        Todo({fg = fg0, gui = styles.bold}), -- LSP
        ...
        LspDiagnosticsUnderlineHint({GruvboxAquaUnderline}), -- nvim-treesitter
        TSNone({}), TSError({Error}), TSTitle({Title}), TSLiteral({String}),
        TSURI({Underlined}), TSVariable({Special}), TSPunctDelimiter({Delimiter}),
        TSPunctBracket({Delimiter}), TSPunctSpecial({Delimiter}),
        TSConstant({Constant}), TSConstBuiltin({Special}), TSConstMacro({Define}),
        TSString({String}), TSStringRegex({String}), TSStringEscape({SpecialChar}),
        TSCharacter({Character}), TSNumber({Number}), TSBoolean({Boolean}),
        TSFloat({Float}), TSFunction({Function}), TSFuncBuiltin({Special}),
        TSFuncMacro({Macro}), TSParameter({Identifier}),
        TSParameterReference({TSParameter}), TSMethod({Function}),
        TSField({Identifier}), TSProperty({Identifier}), TSConstructor({Special}),
        TSAnnotation({PreProc}), TSAttribute({PreProc}), TSNamespace({Include}),
        TSConditional({Conditional}), TSRepeat({Repeat}), TSLabel({Label}),
        TSOperator({Operator}), TSKeyword({Keyword}), TSKeywordFunction({Keyword}),
        TSKeywordOperator({TSOperator}), TSException({Exception}), TSType({Type}),
        TSTypeBuiltin({Type}), TSInclude({Include}), TSVariableBuiltin({Special}),
        TSText({TSNone}), TSStrong({gui = styles.bold}),
        TSEmphasis({gui = italic_strings}), TSUnderline({gui = styles.underline}), -- json
        jsonKeyword({GruvboxGreen}), jsonQuote({GruvboxGreen}),
        jsonBraces({GruvboxFg1}), jsonString({GruvboxFg1}), -- Plugin: start-screen
        StartifyBracket({Delimiter}), StartifyFile({Identifier}),
        StartifyNumber({Number}), StartifyPath({Directory}), StartifySlash({Delimiter}),
        StartifySection({Statement}), StartifySpecial({Comment}),
        StartifyHeader({Title}), StartifyFooter({Title}), StartifyVar({StartifyPath}),
        StartifySelect({Title}), -- Plugin: Telescope
        TelescopeSelection({GruvboxOrangeBold}), TelescopeSlectionCaret({GruvboxRed}),
        TelescopeMultiSelection({GruvboxGray}), TelescopeNormal({GruvboxFg1}),
        TelescopeBorder({TelescopeNormal}), TelescopePromptBorder({TelescopeNormal}),
        TelescopeResultsBorder({TelescopeNormal}),
        TelescopePreviewBorder({TelescopeNormal}), TelescopeMatching({GruvboxBlue}),
        TelescopePromptPrefix({GruvboxRed}), TelescopePrompt({TelescopeNormal}), -- Plugin: signify
        SignifySignAdd({GruvboxGreenSign}), SignifySignChange({GruvboxAquaSign}),
