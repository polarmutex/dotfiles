    ...
    for _, file in ipairs(vim.v.oldfiles) do
        if counter <= 0 then break end
            ...
            if absolute_path and absolute_path ~= '' and not files[absolute_path] and
                string.match(absolute_path, is_dir) == nil and prefix:match_str(absolute_path) ~= nil and not skip(absolute_path) then
...
local sections = {
    {title = "Header", show=header },
    {title = 'Bookmarks', show = bookmarks},
        ...
        if next(section.show) ~= nil then
            ...
            for _, item in ipairs(section.show) do
                ...
                if key ~= nil then
                    ...
                    vim.api.nvim_buf_set_lines(0, linenr + size, linenr + size, false, {'   [' .. key .. ']' .. padding .. item.disp})
                    ...
                    vim.api.nvim_buf_add_highlight(0, -1, 'StartifyBracket', linenr + size, 4 + key_len, 5 + key_len)
                    vim.api.nvim_buf_add_highlight(0, -1, 'StartifyPath', linenr + size, 5 + key_len, -1)
                    table.insert(keybindings, {key = key, cmd = item.cmd})
                    size = size + 1
                else
                    vim.api.nvim_buf_set_lines(0, linenr + size, linenr + size, false, {item.disp})
                    size = size + 1
                end
            end
...
local function setup_keys()
    ...
    for _, binding in ipairs(keybindings) do
        vim.api.nvim_buf_set_keymap(0, 'n', tostring(binding.key), '<cmd>:' .. binding.cmd .. '<cr>', {noremap = true, silent = true})
    end
end
...
local function start_screen()
    ...
    vim.cmd [[silent! setlocal bufhidden=wipe colorcolumn= foldcolumn=0 filetype=startify matchpairs= nocursorcolumn nocursorline nolist nonumber norelativenumber nospell noswapfile signcolumn=no synmaxcol& statusline=]]
    make_sections()
